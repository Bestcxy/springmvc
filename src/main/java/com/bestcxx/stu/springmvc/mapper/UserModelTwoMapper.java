...
public interface UserModelTwoMapper {
	public UserModelTwo getUserModel(@Param("userName") String userName);//搜索返回一个实体,如果有多个则报错
	public int addUserModel(UserModelTwo userModeltwo );//如果启动 mybitas 的属性自识别功能，不要对实体进行@Param("userModel")的设置
	public ArrayList<UserModelTwo> getUserModelListByStrs(@Param("userNames") String userNames);//入参为 11,22,33 形式的字符串，返回实体的list
	public ArrayList<HashMap<String,Object>> getHashMapListByList(@Param("userNamelist") ArrayList userNamelist);
	public void update(UserModelTwo userModel);//update方法
